...
The repo includes a [`render.yaml`](./render.yaml) Blueprint for the web service; the database is [Neon](https://neon.tech) Postgres.

1. Create a Neon project and copy two connection strings from its **Connect** dialog: the **pooled** one (host contains `-pooler`) and the **direct** one (untick "Connection pooling"). Both must include `sslmode=require`.
2. Push this repo to GitHub.
3. In the Render dashboard: **New > Blueprint**, select the repo, and apply.
4. When prompted, set `DATABASE_URL` (pooled), `DIRECT_URL` (direct), and `SEED_DEMO_PASSWORD` (the password for all demo accounts).
5. On each start the service runs `prisma migrate deploy` (using `DIRECT_URL`) and the idempotent seed, then `next start`.

`AUTH_SECRET` is generated by Render, and `AUTH_TRUST_HOST=true` lets Auth.js work behind Render's proxy. Free Render web services sleep when idle.
